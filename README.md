...
- 操作员控制台：<http://127.0.0.1:8080/console>（打开即**新建一个会场**并跳转到 `/<sid>/console`）
- 会场投屏大屏：控制台页脚显示的 `/<sid>/screen`（点击即可复制）；<http://127.0.0.1:8080/screen> 会落到最近会场的投屏页

> **多会场**：每个控制台页面绑定一个会场（sid 写在 URL 里）。同一 `/<sid>/console` 多开＝共享同一份状态（多人看同一个控制台）；
> 不同 sid 完全隔离——各自的转写、AI 输出、投屏与热词互不影响（同时最多 4 个活跃会场）。
