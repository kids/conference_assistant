...
- 会议上下文来自实时语音识别（ASR），可能有同音错字、断句错误、人名/术语误识：
  遇到明显不通顺或用词异常处，按上下文最可能的原意理解；不要把可疑的错词当作新术语去解释或转译。
...
var Tasks = map[string]Task{
	...
	"SUMMARY": {
		...
		Instruction: "请用约 30 秒对刚才的报告做一个通俗易懂的总结，突出关键问题、核心概念与学术意义。" +
			"只依据【本场转写】里报告人实际讲过的内容来总结：不得用讲稿摘要或其它 AI 输出充当报告内容，" +
			"转写中看不出来的信息宁可不说。" +
			"结尾：以上总结请报告人确认。",
		MinChars: 90,
		MaxChars: 150,
...
- 会议上下文是语音识别（ASR）自动转写，细节可能有误：遇到明显不通顺或异常处按最可能的原意理解，
  不要把可疑的错字当成事实依据（人名、机构、数字尤其谨慎）；
